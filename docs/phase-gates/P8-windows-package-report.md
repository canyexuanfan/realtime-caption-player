...
### 补记 12（2026-08-30）：字幕可见性+同步双修 + 开源参考项目补齐
用户反馈"直接看不到字幕"。根因：定稿显示绑在"播放头落在句段窗口"判定上，
worker 离线提取音频领先播放头（实测 67-77s）且可能落后，两种方向都会让
定稿空窗。修复（c9b3442）：只显示播放头已到达的最后一句定稿（startMs<=
head+0.5s，与语音同步）；无已到达句时退回常驻最后到达句（不空窗）；
positionChanged 时刷新选择。50s 长程快照（用户课件 G 盘）：播放头 00:43
显示 ~43s 处句子「并且它是和现在的时期做比较的时期，它的时间相对比较靠前」，
上轮同点显示 111s 句子——同步生效。快照时刻 RCP_SNAP_T1/T2 可覆盖。
同时补齐项目初始化流程欠账：浅克隆 mpv-examples/mpc-qt/QMPlay2/buzz 进
reference/projects/（不进 git），建开源项目索引.md；mpc-qt mpvwidget.cpp:1150/1161
印证我们 FBO/DPR 修复与业界一致。ctest 21/21。MSI 重打（03f30892…）。
